...
contract("LDGRToken", function (accounts) {
  ...
  describe("ERC20 standards", function () {
    ...
    it("should have decimals of 0", function () {
      return deployedToken.decimals.call().then(function (decimals) {
        return decimals.toNumber().should.equal(0);
      });
    });

    it("should have an initial totalSupply of 0", function () {
      return deployedToken.totalSupply.call().then(function (totalSupply) {
  ...
  describe("#burnFrom", function () {
    ...
    beforeEach(function () {
      return deployedToken.mint(someInvestor, initialBalance, {
        from: transferAgent,
      });
    });
  ...
  describe("#transferFrom", function () {
    ...
    beforeEach(function () {
      return deployedToken
        .mint(someInvestor, someInvestorBalance, { from: transferAgent })
        .then(function () {
          return deployedToken.mint(
            someOtherInvestor,
            someOtherInvestorBalance,
            { from: transferAgent }
          );
        });
    });
    ...
    const transferFrom = (from, to, value) => {
      return deployedToken.transferFrom(from, to, value, {
        from: transferAgent,
      });
    };
    ...
    it("should not change totalSupply", function () {
      ...
      return deployedToken.totalSupply
        ...
        .then(function (newBigNumTotalSupply) {
          return newBigNumTotalSupply
            .toNumber()
            .should.equal(initialTotalSupply);
        });
    });
  });
});
