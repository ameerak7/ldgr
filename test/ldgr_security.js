...
contract("LDGRSecurity", function (accounts) {
  ...
  describe("#new", function () {
    ...
    it("should set token to an address", function () {
      return deployedSecurity.token
        .call()
        .should.not.eventually.equal(
          "0x0000000000000000000000000000000000000000"
        );
    });

    it("should deploy contract code to the token address", function () {
      return deployedSecurity.token.call().then(function (token) {
    ...
    it("should emit the CreateToken event with correct address newToken", function () {
      return deployedSecurity.token.call().then(function (token) {
        /*return getEventsByName(deployedSecurity, "CreateToken").then(function (
          events
        ) {
  ...
  describe("#createToken", function () {
    ...
    beforeEach(function () {
      return deployedSecurity
        ...
        .then(function (res) {
          result = res;
          return LDGRToken.at(res.logs[0].args.newToken).then(function (t) {
            token = t;
          });
        });
    });
    ...
    it("should push the newToken address to address[] tokens", function () {
      return deployedSecurity
        .getAllTokens()
        .should.eventually.include(token.address);
    });

    it("should set newToken.issuer to issuer", function () {
      return token.issuer
        .call()
        .should.eventually.equal(deployedIssuer.address);
    });
